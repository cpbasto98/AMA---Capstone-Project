<?php
// login.php
// Place this file inside your logins folder (e.g. C:\xampp\htdocs\barangay_rizal\logins\login.php)

session_start();

// ---------- CONFIG ----------
/*
 If your config.php is in the same folder as this file (logins/config.php),
 keep the path below as-is. If config.php is in the project root (one level up),
 change the path to: __DIR__ . '/../config.php'
*/
$configPath = __DIR__ . '/config.php';
if (!file_exists($configPath)) {
    exit('Missing config.php — create it as instructed.');
}
$config = require $configPath; // expects array with db_host, db_name, db_user, db_pass
...
try {
    ...
    // In production, log $e->getMessage() rather than echoing it.
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($user && password_verify($password, $user['password'])) {
        ...
        // CHANGE THIS if your dashboard is at a different location.
...
<head>
  ...
  <meta name="description" content="Official website of Barangay Rizal, Taguig City.">
  <title>Barangay Rizal • Employee Login</title>
  <link rel="stylesheet" href="../style.css">
  <style>
    /* Small fallback styles so the form looks okay even if style.css path changes */
    .login-card { width:360px; background:#fff; padding:28px; border-radius:8px; box-shadow:0 6px 18px rgba(0,0,0,0.08); }
    .login-card input { width:100%; padding:10px; margin-top:6px; border-radius:6px; border:1px solid #ccc; box-sizing:border-box; }
    .login-card button { width:100%; margin-top:16px; padding:12px; border:none; border-radius:6px; background:#1b5cc6; color:#fff; font-weight:700; cursor:pointer; }
  </style>
</head>
<body>
...
<main>
  <section class="login-section" style="display:flex; justify-content:center; padding:40px 20px;">
    <div class="login-card">
      ...
      <!-- Error message -->
      <?php if (!empty($error)): ?>
        <p style="color:red; text-align:center; margin-bottom:12px;"><?= htmlspecialchars($error, ENT_QUOTES, 'UTF-8') ?></p>
      <?php endif; ?>

      <!-- Login form posts to this same page -->
...
<footer class="footer">
  <div class="footer-container">
    ...
    <div class="footer-column map">
      ...
      <iframe src="https://www.google.com/maps?q=Barangay%20Rizal%20Taguig&output=embed" width="180%" height="180" style="border:0;" allowfullscreen="" loading="lazy"></iframe>
    </div>
  </div>
  ...
  <section class="govph-footer">
    <div class="govph-container">
      ...
      <div class="govph-column">
        ...
        <p>Learn more about Executive Order No. 2 – The order implementing Freedom of Information in the Philippines.</p>
      </div>
      <div class="govph-column">
        <h3>ABOUT GOVPH</h3>
      ...
      <div class="govph-column">
        ...
        <h3>LEGISLATIVE</h3>
        <ul>
          <li><a href="https://senate.gov.ph/" target="_blank">Senate</a></li>
          <li><a href="https://congress.gov.ph/" target="_blank">House</a></li>
        </ul>
      </div>
      <div class="govph-column">
        <h3>JUDICIARY</h3>
    ...
    <p class="govph-bottom">© 2025 Barangay Rizal, Taguig City | Developed by Ian, Kurt and Ranel</p>
  </section>
</footer>
